...
- **단점**: 단일 코어 한계. 12코어 중 1개만 쓰며, 그 1코어가 **700 CCU 부근에서 포화**(틱 p99 33.9ms).
  단 이 700 은 **부하가 맵에 균등 분포할 때**의 숫자다. 유저가 뭉치면 브로드캐스트 팬아웃이
  3.5배로 뛰어 **250~300 CCU** 로 떨어진다. [load-test.md](load-test.md) 참고.
