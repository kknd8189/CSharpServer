...
public class MapManager
{
    ...
    public bool CanGo(Vector3Int cellPos)
	{
        ...
        int z = cellPos.z - MinZ;      // Z축 인덱스 계산

        return gridData[cellPos] == CellType.Empty;
	}
    ...
    public void LoadMap(int mapId)
    {
        ...
        MinX = int.Parse(lines[1]);
        MaxZ = int.Parse(lines[2]);
        MinZ = int.Parse(lines[3]);
    ...
    {
        GameObject oldRoot = GameObject.Find("GeneratedPlaneGrid");
        if (oldRoot != null) GameObject.Destroy(oldRoot);

        cellObjects.Clear();

        GameObject root = new GameObject("GeneratedGrid");
        CurrentGrid = root.AddComponent<Grid>();
        CurrentGrid.cellSize = new Vector3(1, 0, 1);
        ...
        foreach (var kvp in gridData)
        {
            ...
            quad.transform.position = CurrentGrid.CellToWorld(pos);
